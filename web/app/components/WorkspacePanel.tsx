  ...
  if (node.type === "folder") {
    return (
      <div>
        ...
        {open && node.children?.map((c) => <FileTreeItem key={c.path} node={c} depth={depth + 1} />)}
      </div>
    );
  }
  ...
  return (
    <button
      ...
      className={`flex items-center gap-1.5 w-full text-left px-2 py-1 text-xs transition-colors ${
        activeFile === node.path ? "text-white bg-accent/10" : "text-muted hover:text-white"
      }`}
      style={{ paddingLeft: `${depth * 12 + 8}px` }}
    >
      <span>📄</span>
...
type Tab = "editor" | "terminal" | "diffs";
    ...
    <div className="flex h-full overflow-hidden">
      ...
      {files.length > 0 && (
        <div className="w-48 min-w-[160px] border-r border-border overflow-y-auto py-2">
          <p className="px-3 py-1 text-[10px] font-semibold text-muted uppercase tracking-wider">Files</p>
          {files.map((f) => (
          ...
          {tabs.map((t) => (
            <button
              ...
              className={`px-3 py-2 text-xs font-medium transition-colors border-b-2 ${
                tab === t.key ? "text-white border-accent" : "text-muted border-transparent hover:text-white"
              }`}
            >
              {t.label}
              {t.key === "diffs" && diffs.length > 0 && (
                <span className="ml-1.5 text-[10px] bg-accent/20 text-accent px-1.5 py-0.5 rounded-full">{diffs.length}</span>
              )}
            </button>
          ))}
          {testResults && (
            <span className="ml-auto text-xs text-muted px-2">
              ...
              {testResults.failed > 0 && <span className="text-red-400 ml-1">{testResults.failed}✗</span>}
            </span>
          )}
        </div>
          ...
          {tab === "editor" && (
            <div className="h-full">
              {activeFile ? (
                <MonacoEditor
                  height="100%"
                  language={activeFile.endsWith(".tsx") || activeFile.endsWith(".ts") ? "typescript" : activeFile.endsWith(".json") ? "json" : "markdown"}
                  theme="vs-dark"
              ...
              {diffs.length === 0 ? (
                ...
                <>
                  <div className="flex items-center gap-2 mb-2">
                    <span className="text-xs text-emerald-400 font-medium px-2 py-0.5 rounded-full bg-emerald-500/10">Pillar 3: Verification</span>
                  </div>
                  {diffs.map((d, i) => (
                    <div key={i} className="rounded-lg border border-border bg-surface p-3">
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-sm font-medium text-white">{d.file}</span>
                        <span className="text-xs">
                          <span className="text-green-400">+{d.additions}</span>{" "}
                          <span className="text-red-400">-{d.deletions}</span>
                        </span>
                      </div>
                      <pre className="text-xs text-gray-400 font-mono overflow-x-auto whitespace-pre-wrap">{d.content}</pre>
                    </div>
                  ))}
                </>
              )}
